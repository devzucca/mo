...
function makeContainer(scrollTop = 0): HTMLDivElement {
  ...
  el.getBoundingClientRect = () => ({ top: 0, left: 0, right: 800, bottom: 600, width: 800, height: 600, x: 0, y: 0, toJSON() {} });
  return el;
}
...
function addHeading(id: string, topOffset: number) {
  ...
  el.id = id;
  el.getBoundingClientRect = () => ({ top: topOffset, left: 0, right: 800, bottom: topOffset + 30, width: 800, height: 30, x: 0, y: 0, toJSON() {} });
...
describe("useScrollRestoration", () => {
  describe("Path A: heading-based restore", () => {
    it("restores scroll position using heading offset", () => {
      ...
      act(() => { result.current.captureScrollPosition(); });

      // Simulate heading moved after re-render (e.g., content changed)
      heading.getBoundingClientRect = () => ({ top: 150, left: 0, right: 800, bottom: 180, width: 800, height: 30, x: 0, y: 0, toJSON() {} });
      container.scrollTop = 0; // Reset as if re-rendered

      // Trigger restore
      act(() => { result.current.onContentRendered(); });
  ...
  describe("Path A: rawScrollTop fallback", () => {
    it("falls back to rawScrollTop when heading is removed", () => {
      ...
      const { result } = renderHook(
        ({ sc, headingId, fileId }) => useScrollRestoration(sc, headingId, fileId),
        { initialProps: { sc: container as HTMLElement | null, headingId: "temp-heading" as string | null, fileId: "abc" as string | null } },
      );
    ...
    it("uses rawScrollTop when no heading was active", () => {
      ...
      const { result } = renderHook(
        ({ sc, headingId, fileId }) => useScrollRestoration(sc, headingId, fileId),
        { initialProps: { sc: container as HTMLElement | null, headingId: null as string | null, fileId: "abc" as string | null } },
      );
  ...
  describe("Path B: sessionStorage restore on reload", () => {
    ...
    it("skips restore when fileId does not match", () => {
      ...
      sessionStorage.setItem(SCROLL_SESSION_KEY, JSON.stringify({
        headingId: null,
        relativeOffset: 0,
        rawScrollTop: 750,
        fileId: "other-file",
  ...
  describe("captureScrollPosition", () => {
    ...
    it("skips capture when no scroll container", () => {
      const { result } = renderHook(
        ({ sc, headingId, fileId }) => useScrollRestoration(sc, headingId, fileId),
        { initialProps: { sc: null as HTMLElement | null, headingId: null as string | null, fileId: "abc" as string | null } },
      );
